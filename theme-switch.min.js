;(function(){"use strict";function b(d,a,b){if(a in d){Object.defineProperty(d,a,{value:b,enumerable:true,configurable:true,writable:true})}else{d[a]=b}return d}const c=24;const d="#000";const e="theme";const f="auto";const g="dark";const h="light";const i=[f,g,h];const j=h;const k="data-theme";const l="(prefers-color-scheme: dark)";const m=[10,0,33,0];const n=[10,0,20,1];const o=[5,1,33,1];class p extends HTMLElement{constructor(){super();b(this,"shadowRoot",void 0);b(this,"themeToggleEvent",void 0);b(this,"identifier",p.counter++);this.shadowRoot=this.attachShadow({mode:"open"});this.shadowRoot.innerHTML=q(...v());this.shadowRoot.host.addEventListener("click",()=>{this.toggleTheme();this.dispatchEvent(this.themeToggleEvent)});document.addEventListener("themeToggle",b=>{if(b.detail!==this.identifier){this.reflectTheme()}});this.themeToggleEvent=new CustomEvent("themeToggle",{detail:this.identifier,bubbles:true,composed:true,cancelable:false});const c=document.createElement("style");c.textContent=r();this.shadowRoot.append(c)}toggleTheme(){const b=t();if(b===f){localStorage.setItem(e,h);this.animateThemeButtonIconToLight()}else if(b===g){localStorage.setItem(e,f);this.animateThemeButtonIconToAuto()}else{localStorage.setItem(e,g);this.animateThemeButtonIconToDark()}s()}reflectTheme(){const b=t();if(b===f){this.animateThemeButtonIconToAuto()}else if(b===g){this.animateThemeButtonIconToDark()}else{this.animateThemeButtonIconToLight()}}animateThemeButtonIconToLight(){this.shadowRoot.getElementById("letter-anim-hide").beginElement();this.shadowRoot.getElementById("core-anim-shrink").beginElement();this.shadowRoot.getElementById("rays-anim-rotate").beginElement();this.shadowRoot.getElementById("rays-anim-show").beginElement()}animateThemeButtonIconToAuto(){this.shadowRoot.getElementById("eclipse-anim-go").beginElement();this.shadowRoot.getElementById("letter-anim-show").beginElement()}animateThemeButtonIconToDark(){this.shadowRoot.getElementById("rays-anim-hide").beginElement();this.shadowRoot.getElementById("core-anim-enlarge").beginElement();this.shadowRoot.getElementById("eclipse-anim-come").beginElement()}}b(p,"counter",0);function q(e,a,b,c){return`
    ...
    `}s();window.customElements.define("theme-switch",p);window.matchMedia(l).addEventListener("change",s);function s(){let b=t();if(b===f)b=u();document.documentElement.setAttribute(k,b)}function t(){const b=localStorage.getItem(e);return i.includes(b)?b:j}function u(){const b=window.matchMedia(l).matches;return b?g:h}function v(){const b=t();if(b===f){return m}else if(b===g){return n}else{return o}}})();
