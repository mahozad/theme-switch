;(function(){"use strict";function b(d,a,b){if(a in d){Object.defineProperty(d,a,{value:b,enumerable:true,configurable:true,writable:true})}else{d[a]=b}return d}const c=24;const d="#000";const e="theme";const f="auto";const g="dark";const h="light";const i=[f,g,h];const j=h;const k="data-theme";const l="(prefers-color-scheme: dark)";const m="themeToggle";const n=[10,0,33,0];const o=[10,0,20,1];const p=[5,1,33,1];class q extends HTMLElement{constructor(){super();b(this,"shadowRoot",void 0);b(this,"identifier",q.counter++);this.shadowRoot=this.attachShadow({mode:"open"});this.shadowRoot.innerHTML=r(...w());this.shadowRoot.host.addEventListener("click",()=>{const d=u();this.toggleTheme(d);const a=u();const b=new CustomEvent(m,{detail:{originId:this.identifier,oldState:d,newState:a},bubbles:true,composed:true,cancelable:false});this.dispatchEvent(b)});document.addEventListener(m,b=>{if(b.detail.originId!==this.identifier){this.reflectTheme()}});const c=document.createElement("style");c.textContent=s();this.shadowRoot.append(c)}toggleTheme(b){if(b===f){localStorage.setItem(e,h);this.animateThemeButtonIconToLight()}else if(b===g){localStorage.setItem(e,f);this.animateThemeButtonIconToAuto()}else{localStorage.setItem(e,g);this.animateThemeButtonIconToDark()}t()}reflectTheme(){const b=u();if(b===f){this.animateThemeButtonIconToAuto()}else if(b===g){this.animateThemeButtonIconToDark()}else{this.animateThemeButtonIconToLight()}}animateThemeButtonIconToLight(){this.shadowRoot.getElementById("letter-anim-hide").beginElement();this.shadowRoot.getElementById("core-anim-shrink").beginElement();this.shadowRoot.getElementById("rays-anim-rotate").beginElement();this.shadowRoot.getElementById("rays-anim-show").beginElement()}animateThemeButtonIconToAuto(){this.shadowRoot.getElementById("eclipse-anim-go").beginElement();this.shadowRoot.getElementById("letter-anim-show").beginElement()}animateThemeButtonIconToDark(){this.shadowRoot.getElementById("rays-anim-hide").beginElement();this.shadowRoot.getElementById("core-anim-enlarge").beginElement();this.shadowRoot.getElementById("eclipse-anim-come").beginElement()}}b(q,"counter",0);function r(e,a,b,c){return`
    ...
    `}t();window.customElements.define("theme-switch",q);window.matchMedia(l).addEventListener("change",t);function t(){let b=u();if(b===f)b=v();document.documentElement.setAttribute(k,b)}function u(){const b=localStorage.getItem(e);return i.includes(b)?b:j}function v(){const b=window.matchMedia(l).matches;return b?g:h}function w(){const b=u();if(b===f){return n}else if(b===g){return o}else{return p}}})();
